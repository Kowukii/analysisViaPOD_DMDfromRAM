...
def read_data3(path):
    df = pd.read_table(path, header=None, sep='\s+', engine='python', skipinitialspace=True)
    df.columns = df.iloc[0]
    df = df[1:]
    return df
def collect_data2(df):
    columns = ['vorticity-mag', 'velocity-magnitude', 'velocity-angle', 'x-coordinate', 'y-coordinate']
    data = []
#     print(df.columns)
    for col in columns:
        if col not in df.columns:
            data.append(np.zeros_like(df.index, dtype=float))
            print(col, 'is not in')
        else:
            data.append(df[col].astype(float))

    vort, v_m, v_a, x_grid, y_grid = data

    # 计算速度的 x 和 y 分量
    vx = v_m * np.cos(v_a)
    vy = v_m * np.sin(v_a)
#     print('output:',df, 'with collect_data2')
...
#         print('i:', i)
        df = read_data3(file_name)
        if i == 0:
            x_grid, y_grid, vx, vy, vort = collect_data2(df)
            Vx = vx
            Vy = vy
            VortAll = vort
        else:
            _, _, vx, vy, vort = collect_data2(df)
...
def create_moden_streamplot(n, k):
    Uxyi, Vxyi = UV2UxyVxy(PhiU[:, n-1], Ny, Nx)
    
    extend_X = np.concatenate((np.flip(-Y), Y))
    extend_Y = np.concatenate((np.flip(X), X))
    extend_Vxyi_quiver = np.concatenate((np.flip(Vxyi), Vxyi), axis=0)
    extend_Uxyi_quiver = np.concatenate((np.flip(Uxyi), Uxyi), axis=0)
    
    y_extend = np.concatenate((np.flip(-y)[0:y.shape[0]-1], y))
    x_extend = x
#     extend_Vxyi_streamplot = np.concatenate((np.flip(-Vxyi, axis=1)[0:Vxyi.shape[0]-1], Vxyi), axis=0)
#     extend_Uxyi_streamplot = np.concatenate((np.flip(Uxyi, axis=1)[0:Uxyi.shape[0]-1], Uxyi), axis=0)
    extend_Vxyi_streamplot = np.concatenate((np.flip(-Vxyi[0:Uxyi.shape[0]-1].T, axis=1), Vxyi.T), axis=1)
    extend_Uxyi_streamplot = np.concatenate((np.flip(Uxyi[0:Uxyi.shape[0]-1].T, axis=1), Uxyi.T), axis=1)
    
    fig0 = plt.figure()
    
    mycmap=plt.get_cmap('rainbow')
#     mynorm=matplotlib.colors.BoundaryNorm(range(0,81,5),ncolors=mycmap.N,clip=True)
    plt.quiver(extend_X[::k, ::k], extend_Y[::k, ::k],
               extend_Vxyi_quiver[::k, ::k], 
               extend_Uxyi_quiver[::k, ::k], cmap=mycmap, scale=None, alpha=0.6)
    plt.streamplot(y_extend, x_extend, extend_Vxyi_streamplot, extend_Uxyi_streamplot, density=2, linewidth=0.5)
#     plt.streamplot(y, x, Vxyi.T, Uxyi.T, density=2, linewidth=0.5, cmap='viridis')
#     plt.streamplot(np.flip(-y), x, np.flip(-Vxyi.T, axis=1), np.flip(Uxyi.T, axis=1), density=3, linewidth=0.5, cmap='viridis')
    plt.savefig("results/mirror/mode"+str(n)+"flow_pattern.png", dpi=300)
    plt.show()
